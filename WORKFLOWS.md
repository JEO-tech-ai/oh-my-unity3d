...
JEO 오케스트레이션 + bmad-gds + unity-mcp 통합 워크플로우 가이드

## 워크플로우 개요

모든 워크플로우는 `jeo` 명령으로 시작하며, JEO가 PLAN → EXECUTE → VERIFY → CLEANUP 단계를 오케스트레이션합니다.
...
### 빠른 실행 예제
```bash
jeo "에셋 파이프라인: 캐릭터 텍스처 배치 임포트 및 머티리얼 설정"
...
### 빠른 실행 예제
```bash
jeo "성능 최적화: 모바일에서 프레임 드랍 30fps 이하"
# → 콘솔 분석 → 병목 탐색 → 최적화 → 회귀 테스트
```

---

## JEO Unity3D 검증 루프 상세
